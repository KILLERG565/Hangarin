...
/* ─── API BASE URL ───────────────────────────────────────────
   Always use /api — Vite proxy handles it in dev, same origin in prod */
const API = "/api";
...
export default function App() {
  ...
  return (
    <>
      ...
      {/* Recipe Detail Modal */}
      {activePost && (
        <div onClick={() => setActivePost(null)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.55)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16, backdropFilter:"blur(4px)" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"white", borderRadius:20, width:"100%", maxWidth:700, maxHeight:"92vh", overflowY:"auto", boxShadow:"0 20px 80px rgba(0,0,0,.3)" }}>
            <div style={{ background:"linear-gradient(135deg,#c8533a,#a03a25)", padding:"24px 28px", position:"relative" }}>
              <button onClick={() => setActivePost(null)} style={{ position:"absolute", top:16, right:16, background:"rgba(255,255,255,.2)", border:"none", color:"white", borderRadius:"50%", width:32, height:32, cursor:"pointer", fontSize:18, display:"flex", alignItems:"center", justifyContent:"center" }}>×</button>
              <div style={{ fontFamily:"'Fraunces',serif", fontSize:28, color:"white", marginBottom:8 }}>{activePost.title}</div>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <Avatar name={activePost.author} size={40} />
                <div style={{ color:"#ffcfb0" }}>
                  <div style={{ fontSize:14, fontWeight:700 }}>{activePost.author}</div>
                  <div style={{ fontSize:12 }}>{activePost.category} • {activePost.cook_time} • Serves {activePost.serves}</div>
                </div>
              </div>
            </div>
            <div style={{ padding:28 }}>
              <p style={{ fontSize:15, color:"#7a6a5a", lineHeight:1.8, marginBottom:24 }}>{activePost.description}</p>
              
              {/* Ingredients */}
              <div style={{ marginBottom:28 }}>
                <h3 style={{ fontFamily:"'Fraunces',serif", fontSize:20, color:"#1a1005", marginBottom:12 }}>Ingredients</h3>
                <ul style={{ listStyle:"none" }}>
                  {(activePost.ingredients || []).map((ing, i) => (
                    <li key={i} style={{ padding:"6px 0", fontSize:14, color:"#5a4a3a", display:"flex", gap:10 }}>
                      <span style={{ color:"#c8533a", fontWeight:700, minWidth:20 }}>•</span>
                      <span>{typeof ing === 'string' ? ing : ing.text}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {/* Steps */}
              <div style={{ marginBottom:28 }}>
                <h3 style={{ fontFamily:"'Fraunces',serif", fontSize:20, color:"#1a1005", marginBottom:12 }}>Instructions</h3>
                <ol style={{ listStyle:"none", counterReset:"step-counter", paddingLeft:0 }}>
                  {(activePost.steps || []).map((step, i) => (
                    <li key={i} style={{ padding:"10px 0", fontSize:14, color:"#5a4a3a", display:"flex", gap:14, lineHeight:1.6, marginBottom:8 }}>
                      <span style={{ background:"#c8533a", color:"white", minWidth:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, flexShrink:0 }}>{i+1}</span>
                      <span style={{ paddingTop:2 }}>{typeof step === 'string' ? step : step.text}</span>
                    </li>
                  ))}
                </ol>
              </div>
              ...
              <div style={{ borderTop:"1px solid #f0e8df", paddingTop:20 }}>
                <h3 style={{ fontFamily:"'Fraunces',serif", fontSize:18, color:"#1a1005", marginBottom:16 }}>Reviews ({(activePost.comments || []).length})</h3>
                {(activePost.comments || []).length === 0 ? (
                  <div style={{ fontSize:14, color:"#c0b0a0", fontStyle:"italic", padding:"20px 0" }}>No reviews yet. Be the first!</div>
                ) : (
                  <div style={{ maxHeight:300, overflowY:"auto" }}>
                    {(activePost.comments || []).map(c => (
                      <div key={c.id} style={{ display:"flex", gap:10, padding:"12px 0", borderBottom:"1px solid #f5ede4" }}>
                        <Avatar name={c.author} size={32} />
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:13, fontWeight:700, color:"#2c1f0f" }}>{c.author} <span style={{ color:"#c0b0a0", fontWeight:400, fontSize:12 }}>{new Date(c.created_at).toLocaleDateString()}</span></div>
                          <div style={{ fontSize:13, color:"#5a4a3a", marginTop:4, lineHeight:1.5 }}>{c.text}</div>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {/* Post Form Modal */}
      {showForm && <PostForm onSubmit={handlePost} onClose={() => setShowForm(false)} />}
    </>
  );
}
        ...
        <div style={{ background:"#fdfaf6", borderRadius:10, padding:"12px 14px", marginBottom:16, border:"1px solid #f0e8df" }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
            {(post.ingredients || []).slice(0,5).map((ing, i) => {
              ...
              return <span key={i} style={{ background:"white", border:"1px solid #e8ddd0", borderRadius:6, fontSize:11.5, padding:"3px 8px", color:"#5a4a3a" }}>{text.split(",")[0].split("(")[0].trim()}</span>;
            })}
        ...
        <div style={{ padding:"0 20px 16px", borderTop:"1px solid #f5ede4" }}>
          ...
          {(post.comments || []).map(c => (
            <div key={c.id} style={{ display:"flex", gap:10, padding:"10px 0", borderBottom:"1px solid #f5ede4" }}>
              ...
              <div>
                <div style={{ fontSize:12, fontWeight:700, color:"#2c1f0f", fontFamily:"'DM Sans',sans-serif" }}>{c.author} <span style={{ color:"#c0b0a0", fontWeight:400 }}>{new Date(c.created_at).toLocaleDateString()}</span></div>
